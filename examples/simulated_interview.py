"""模拟访谈演示：脚本化"老人"与访谈 Agent 走完 A（深挖）→ B（切换）→ C（收尾）全流程。

运行：PYTHONPATH=src python3 examples/simulated_interview.py
...
# 脚本化老人：每轮回答 + 期望分支（注释），覆盖 A→A→A→B→A→A→B→A→C
SCRIPT = [
    "我在四川嘉陵江边长大的，经常去游泳。有一次差点被水冲走，多亏邻居王二哥把我捞起来。",  # A 深挖
    "那是七八岁的时候，水流特别急，我越扑腾离岸越远，吓得直哭。",  # A 深挖
    "后来王二哥跳下水，一把抓住我的胳膊，把我拖上了岸。我娘知道后，一边哭一边给我煮姜汤。",  # A 深挖
    "记不清了，都是很久以前的事了。",  # B 切换 → 故乡
    "我们村口有棵大槐树，全村人夏天都在树下乘凉，村长还常在那儿给大家开会。",  # A 深挖
    "有一年夏天特别热，树下挤了几十口人，村长王老三给大家讲古，我听得入了迷，连晚饭都忘了吃。",  # A 深挖
    "别的也记不清了。",  # B 切换 → 求学
    "我只读了三年书，私塾先生姓陈，特别严厉。有一次我逃学去掏鸟窝，被他拿戒尺打了手心。",  # A 深挖
    "今天有点累了，咱们下次再聊吧。",  # C 收尾
]
...
def main() -> None:
    ...
    print("=" * 72)
    print("Phase 1 初次访谈 Agent · 模拟演示（Mock/真模型均可跑）")
    print("=" * 72)
    ...
    for i, answer in enumerate(SCRIPT, 1):
        ...
        d = reply.decision
        if d:
            print(f"  🔀 决策：{BRANCH_CN[d.action.value]}（置信度 {d.confidence}）")
            print(f"     理由：{d.reasoning}")
            if reply.new_fragments:
